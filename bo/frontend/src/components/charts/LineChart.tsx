...
const LineChart: React.FC<LineChartProps> = ({ data, config }) => {
  ...
  // Generate lines for each measure in the config
  const lines = useMemo(() => {
    ...
    return config.measures.map((measure, index) => {
      ...
      return (
        <Line
          ...
          type={config.chartSubtype === 'curved' ? 'monotone' : 'linear'}
          dataKey={measure.field.id}
          name={measure.label || measure.field.name}
          stroke={measure.color || colors[index % colors.length]}
          dot={config.chartSubtype === 'dotted' ? { fill: measure.color || colors[index % colors.length] } : false}
          activeDot={{ r: 6 }}
        />
      );
    });
