...
import * as dataObjectsApi from '../api/dataObjectsApi';
...
const DataObjectsCanvas: React.FC = () => {
  ...
  const fetchTableSchemas = async (dataSourceId: string) => {
    try {
      ...
      setError({
        ...
        details: err instanceof Error ? err.message : 'Please check your connection and try again.',
        retry: () => fetchTableSchemas(dataSourceId)
      });
      setLoading(false);
    }
  };
  ...
  const fetchSavedModels = async () => {
    try {
      ...
      setError({
        message: 'Failed to fetch saved data models.',
  ...
  const loadDataModel = async (modelId: string) => {
    try {
      ...
      setError({
        message: `Failed to load data model.`,
        details: err instanceof Error ? err.message : undefined
      });
      setLoading(false);
    }
  ...
  const handleDataSourceChange = (event: React.ChangeEvent<{ value: unknown }>) => {
    ...
    if (newDataSourceId !== selectedDataSource?.id) {
      // Reset the canvas when changing data sources
      setNodes([]);
      setEdges([]);
      setSelectedTables([]);
      setTables([]);
      ...
      selectDataSource(newDataSourceId);
    }
  };
        ...
        <Box sx={{ p: 2 }}>
          ...
          <FormControl fullWidth variant="outlined" size="small" sx={{ mb: 2 }}>
            ...
            <Select
              ...
              onChange={(e) => handleDataSourceChange(e as any)}
              label="Data Source"
              startAdornment={<DataObjectIcon sx={{ mr: 1, color: 'text.secondary' }} />}
            >
          ...
          {error && (
            <Alert 
              ...
              action={
                error.retry && (
                  <Button color="inherit" size="small" onClick={error.retry}>
                    Retry
                  </Button>
                )
              }
          ...
          {!loading && !error && tables.length === 0 && selectedDataSource && (
            <Alert severity="info" sx={{ mb: 2 }}>
              No tables found in this data source. Please check your connection configuration.
            </Alert>
          )}
          
          {dataSources.length === 0 && !dataSourceLoading && (
            <Alert severity="info" sx={{ mb: 2 }}>
