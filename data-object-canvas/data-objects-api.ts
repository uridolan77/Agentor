...
const API_BASE_URL = process.env.REACT_APP_API_BASE_URL || '';  // Removed '/api' prefix
...
export const createDataModel = async (dataModel: Partial<DataModel>): Promise<DataModel> => {
  try {
    ...
    return handleApiError(error, 'Failed to create data model');
  }
};
...
export const updateDataModel = async (modelId: string, dataModel: Partial<DataModel>): Promise<DataModel> => {
  try {
    ...
    return handleApiError(error, `Failed to update data model`);
  }
};
...
export const getDataModels = async (dataSourceId: string): Promise<DataModel[]> => {
  try {
    ...
    return handleApiError(error, 'Failed to fetch data models');
  }
};
...
export const getDataModel = async (modelId: string): Promise<DataModel> => {
  try {
    ...
    return handleApiError(error, `Failed to fetch data model`);
  }
};
...
export const deleteDataModel = async (modelId: string): Promise<void> => {
  try {
    ...
    return handleApiError(error, `Failed to delete data model`);
  }
};
...
export const generateSqlFromModel = async (modelId: string, options: { 
  ...
  try {
    ...
    return handleApiError(error, `Failed to generate SQL for model ${modelId}`);
  }
};
...
export const testQuery = async (dataSourceId: string, query: string): Promise<any> => {
  try {
    ...
    return handleApiError(error, 'Failed to test query');
  }
};

/**
 * Test database connection for a data source
 */
export const testDatabaseConnection = async (dataSourceId: string): Promise<{ success: boolean; message: string }> => {
  try {
    const response = await api.post(`/reporting/datasources/${dataSourceId}/test-connection`);
    return {
      success: true,
